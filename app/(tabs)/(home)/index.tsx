...
export default function HomeScreen() {
  ...
  const loadItems = useCallback(async () => {
    try {
      console.log('[HomeScreen] Loading pantry items');
      const items = await loadPantryItems();
      console.log('[HomeScreen] Loaded', items.length, 'items');
      setPantryItems(items);
    } catch (error) {
      console.error('[HomeScreen] Error loading pantry items:', error);
    }
  }, []);

  useFocusEffect(
    useCallback(() => {
      console.log('[HomeScreen] Screen focused, loading items');
      loadItems();
    }, [loadItems])
  );
  ...
  const handleAddItem = () => {
    console.log('[HomeScreen] User tapped Add Item button');
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    router.push('/add-item');
  };

  const handleAIRecipes = () => {
    console.log('[HomeScreen] User tapped AI Recipe Generator button');
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    router.push('/ai-recipes');
  };

  const handleScanReceipt = () => {
    console.log('[HomeScreen] User tapped Scan Receipt button');
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    router.push('/scan-receipt');
  };

  const handleExpiringItem = (itemId: string) => {
    console.log('[HomeScreen] User tapped expiring item:', itemId);
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
          ...
          <View style={styles.alertSection}>
            ...
            {expiringItems.slice(0, 3).map((item, index) => (
              <TouchableOpacity
                ...
                onPress={() => handleExpiringItem(item.id)}
              >
